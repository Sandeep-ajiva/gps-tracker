...
export default function DashboardPage() {
  ...
  const { data: liveData = [] } = useGetLiveVehiclesQuery({});

  const onlineVehicles = liveData.filter((v: any) => v.status === "online").length;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 font-bold">
          ...
          <div className="space-y-6">
            <StatusItem label="Total Fleet" value={vehicles.length} color="bg-gray-200" />
            ...
            <StatusItem label="Unassigned Devices" value={devices.length - vehicles.length} color="bg-yellow-500" />
          </div>
        </div>
      </div>
    </div>
