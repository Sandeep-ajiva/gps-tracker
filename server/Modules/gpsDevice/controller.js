...
const validateGpsDevice = async (data, isUpdate = false) => {
  const rules = {
    ...
    model: isUpdate ? "string|min:2" : "required|string|min:2",
    vendor: "string",
    simNumber: "string",
  };
...
exports.create = async (req, res) => {
  try {
    ...
    const existing = await GpsDeviceModel.findOne({ imei: req.body.imei });
    if (existing) {
...
exports.getAvailable = async (req, res) => {
  try {
    const filter = { status: "stock", isActive: true };
...
exports.update = async (req, res) => {
  try {
    ...
    // Prevent IMEI duplication
    if (req.body.imei && req.body.imei !== device.imei) {
      const exists = await GpsDeviceModel.findOne({ imei: req.body.imei });
      if (exists) {
        return res.status(400).json({ message: "IMEI already exists" });
      }
    }

    const allowedFields = ["imei", "model", "vendor", "simNumber", "manufacturer"];
...
//     if (!device.isActive || device.status !== "stock") {
...
exports.deactivate = async (req, res) => {
  try {
    ...
    if (device.status === "assigned") {
      return res
        .status(400)
        .json({ message: "Unassign device before deactivation" });
    }
